 ...
 * Author:  Leslie
 * 网络请求管理者，根据 okHttp 封装的。
 * 参考地址：http://www.cnblogs.com/whoislcj/p/5526431.html
 */
...
public class NetRequestManager {
    ...
    private static volatile NetRequestManager mInstance;                                            //单利引用
    //----------------------------------------------------------------------------------------------
    ...
    public NetRequestManager(Context context) {
        ...
        mOkHttpClient = new OkHttpClient().newBuilder()
                .connectTimeout(CONNECT_TIMEOUT, TimeUnit.SECONDS)                                  //设置超时时间
    ...
    public static NetRequestManager getInstance(Context context) {
        NetRequestManager inst = mInstance;
        if (inst == null) {
            synchronized (NetRequestManager.class) {
                inst = mInstance;
                if (inst == null) {
                    inst = new NetRequestManager(context.getApplicationContext());
                    mInstance = inst;
                }
            }
        }
